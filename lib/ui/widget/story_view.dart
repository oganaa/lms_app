...
class StoryItem {
  ...
  static StoryItem text(
    ...
    return StoryItem(
        ...
        shown: shown, titleText: title);
  }

  static StoryItem mongoltext(
      String title,
      Color backgroundColor, {
        bool shown = false,
        double fontSize = 35,
        ...
        Container(
          ...
          child: Center(
            ...
            /*new Container(
              ...
              child: title.length == null ? null : new AnimatedBuilder(
                animation: _characterCount,
                builder: (BuildContext context, Widget child) {
                  String text = title.substring(0, _characterCount.value);
                  return
                    new MongolText(
                        title,
                        style: TextStyle(
  ...
  static StoryItem pageImage(
    ...
    BoxFit imageFit = BoxFit.fitWidth,
        ...
        double fontSizee = 30,
        String caption = null,
        String captionM = null,
        String fontNameM = null,
    bool shown = false,
  }) {
    ...
    return StoryItem(
        Container(
          ...
          child: Stack(
            children: <Widget>[
              Align(
                alignment:  captionM!=null ? Alignment.centerLeft : Alignment.center, // imageWidth == null ? Alignment.center : ( captionM == null && caption == '' ? Alignment.center : Alignment.centerLeft),
                child: Image(
                  image: image,
                  height: double.infinity,
                  width: imageWidth == null ?double.infinity : imageWidth,
                  fit: imageFit,
                ),
              ),
              SafeArea(
                child: Align(
                  alignment: captionM != null ? Alignment.center : Alignment.bottomCenter,
                  child:
                  captionM == null ?
                  ( caption == null ? SizedBox() :
                  Container(
                    ...
                    color:
                        caption != null ? backgroundColor : Colors.transparent,
                    child: caption != null
                        ? Text(
                            caption,
                            style: TextStyle(
                              fontSize: 20,
                              color: contrast > 1.8 ? Colors.white : Colors.black, //Colors.white,
                            ),
                            textAlign: TextAlign.center,
                          )
                        : SizedBox(),
                  ...
                  Container(
                      width: double.infinity,
                      margin: EdgeInsets.only(
                        left: imageWidth== null ? 100 : imageWidth + 10,
                        bottom: 24,
                      ),
                      padding: EdgeInsets.symmetric(
                        horizontal: imageWidth == null ? 40 : 0,
                        vertical: 20,
                      ),
                      color:
                      captionM != null ? backgroundColor : Colors.transparent,
                      child: captionM != null
                          ? MongolText(captionM,
                            style: TextStyle(
                                fontSize: fontSizee,
                                color: contrast > 1.8 ? Colors.white : Colors.black,
                                fontFamily: fontNameM
                              ),
                            )
                          : SizedBox()
                  ),
        ...
        shown: shown, titleText: captionM == null ? caption : captionM  );
  }
  ...
  static StoryItem inlineImage(
    ...
    bool roundedBottom = false,
        Alignment txtAlign = Alignment.topCenter
  }) {
    return StoryItem(
      Container(
        decoration: BoxDecoration(
        ...
        child: Container(
          ...
          padding: EdgeInsets.symmetric(
            horizontal: 24,
            vertical: captionM == null ? 8 : 30,
          ),
          child:
           Container(
              child:
              Align(
                alignment: txtAlign,//captionM != null ? Alignment.topCenter : Alignment.bottomLeft,
                child:
                caption == null ?
                    ( captionM == null ? SizedBox() :captionM ) : caption,
            ),
             width: double.infinity,
          ),
        ),
...
class StoryViewState extends State<StoryView> with TickerProviderStateMixin {
  ...
  Animation<int> _characterCount;
  int _stringIndex;
  AnimationController txtController;
  ...
  void play() {
    ...
    //print('Play storyItem = '+storyItem.titleText );
    ...
    AnimationController txtController = new AnimationController(
      duration: const Duration(milliseconds: 3000),
      vsync: this,
    );
    setState(() {
      _stringIndex = _stringIndex == null ? 0 : _stringIndex + 1;
      _characterCount = new StepTween(begin: 0, end: storyItem.titleText.length ) //0_curre ntString.length
          .animate(new CurvedAnimation(parent: txtController, curve: Curves.easeIn));
    });
    txtController.forward().then((f){
            ...
            child: RawGestureDetector(
              gestures: <Type, GestureRecognizerFactory>{
                TapGestureRecognizer:
                  ...
                  instance
                    ..onTapDown = (details) {
                      if( !(widget.inline && this._stringIndex==0 ))  {
                        controlPause();
                        debouncer?.cancel();
                        debouncer = Timer(Duration(milliseconds: 500), () {});
                      }
                    ...
                    ..onTapUp = (details) {
                      ...
                      else if (debouncer?.isActive == true ) {
                        debouncer.cancel();
                        debouncer = null;
